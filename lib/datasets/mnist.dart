...
class Mnist {
  ...
  Future<Tuple2<List<List<double>>, List<int>>> readTrainRandom() async {
  ...
  Future<Tuple2<List<List<double>>, List<int>>> readTestRandom() async {
  ...
  Future<Tuple2<List<List<double>>, List<int>>> _readDataRandom(
      String filename) async {
    // read file as stream because of file size
    final file = File(filename);
    Stream<String> lines = file
    ...
    try {
      await for (var line in lines) {
        ...
        if (lineCount != 0) {
          ...
          images.add(imageProcessor.transformImage(
            image,
