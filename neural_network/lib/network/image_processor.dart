...
class ImageProcessor {
  NNImage transformImage(NNImage original, TransformationSettings settings) {
    ...
    if (settings.scale != 0) {
      Tuple<double, double> iHat = Tuple(
        v1: math.cos(settings.angle) / settings.scale,
        v2: math.sin(settings.angle) / settings.scale,
      );
      Tuple<double, double> jHat = Tuple(v1: -iHat.v2, v2: iHat.v1);
...
class TransformationSettings {
  late double angle;
  late double scale;
  late Tuple<double, double> offset;
  late int noiseSeed;
  ...
  TransformationSettings.random({int size = 28}) {
    ...
    offset = Tuple(v1: rng.nextInt(15) / 100, v2: rng.nextInt(15) / 100);
    if (rng.nextBool()) {
    ...
    // offset = Tuple(v1: offsetX * 0.8, v2: offsetY * 0.8);
  }
